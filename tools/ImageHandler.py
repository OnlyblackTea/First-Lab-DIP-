...
class ImageHandler:
    ...
    def __init__(self, image_path, _convert):
        self.image_name = image_path.rsplit('.')[0]
        self.im = Image.open(image_path).convert(_convert)
        # 将图片转化为矩阵处理
        self.image_matrix = np.array(self.im)
        self.image_df = pd.DataFrame(self.image_matrix)
    ...
    def save(self):
        image = Image.fromarray(self.output_matrix)
        image.save(f'{self.image_name}_edited.jpg')
